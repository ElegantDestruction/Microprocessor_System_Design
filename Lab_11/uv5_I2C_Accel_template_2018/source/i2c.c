...
void i2c_read_setup(uint8_t dev, uint8_t address) //parameters are device and its address; both are uint8_t type
{
   ...
   I2C_WAIT;					 /*wait for completion */
	...
	I2C0->D = (dev|0x01);	/*send dev address and read command	(0x01 for reading)*/ 
	I2C_WAIT;							/*wait for completion */
...
{
	uint8_t data;				//Declare dummy variable
	
	if (isLastRead)
	{                   /*If this is the last READ (isLastRead); if no, send NACK; if yes, send ACK*/
...
uint8_t i2c_read_byte(uint8_t dev, uint8_t address)
{
	...
	I2C_M_RSTART; 				/*repeated start */
	I2C0->D = (dev|0x01);	/*3. send dev address (read)	*/
	I2C_WAIT;							/*wait for completion */
